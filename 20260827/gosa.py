...
def solve_inverse_problem():
    ...
    print("【逆問題 フィッティング結果】")
    print(f"{'Ch':>3} | {'v0 (m/s)':>10} | {'dV (m/s)':>10} | {'dV_err (m/s)':>14} | {'A':>10} | {'bg':>10}")
    print("-" * 75)
    ...
    for ch in range(num_channels):
        ...
        try:
            ...
            # pcov の (2,2) 成分 = dV の分散 → 平方根で標準誤差 (1σ)
            ...
            print(f"{ch+1:>3} | {v0_fit:>10.1f} | {dV_fit:>10.1f} | {dV_err:>14.1f} | {A_fit:>10.1f} | {bg_fit:>10.1f}")
    ...
    dv_err_valid = [res['dV_err'] / 1000.0 for res in results if not res['error']] # km/s に変換
    ...
    # Thermal Width (dV)
    ax_dv.errorbar(r_valid, dv_valid, yerr=dv_err_valid, fmt='s-', color='orange',
                   linewidth=2, markersize=8, capsize=4, ecolor='gray', label='dV ± 1σ')
    ...
    with open(csv_filename, 'w', newline='', encoding='utf-8') as f:
        ...
        writer.writerow(['Channel', 'R (m)', 'v0 (km/s)', 'dV (m/s)', 'dV_err (m/s)', 'A'])
        for ch, r, v0, dv, dv_err, a in zip(ch_valid, r_valid, v0_valid, dv_valid, dv_err_valid, A_valid):
            # dv_valid, dv_err_valid は km/s なので 1000 を掛けて m/s に戻す
            writer.writerow([ch, f"{r:.4f}", f"{v0:.4f}", f"{dv * 1000.0:.4f}", f"{dv_err * 1000.0:.4f}", f"{a:.4f}"])
